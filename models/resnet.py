...
class TempScaling(torch.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, logits, get_temps=False):
        temps = torch.exp(torch.clamp(logits[:,-1:], min=-6, max=6))
        logits = logits[:,:-1]*temps
        if get_temps:
            return logits, temps
        return logits

class TempScaledResNet(Classifier):
    """
    Simple classifier.
    """
    ...
    def __init__(self, N_class, resolution=(1, 32, 32), blocks=[3, 3, 3], normalization=True, channels=64, **kwargs):
        ...
        super(TempScaledResNet, self).__init__(N_class, resolution, **kwargs)
        ...
        gain = torch.nn.init.calculate_gain('relu')
        logits = torch.nn.Linear(representation, self._N_output+1)
        torch.nn.init.kaiming_normal_(logits.weight, gain)
        torch.nn.init.constant_(logits.bias, 0)
        #temp =  TempScaling()
        self.append_layer('logits', logits)
        #self.append_layer('logits', temp)
        
    def forward(self, imgs, get_temps=False):
        logits = super().forward(imgs)
        temps = torch.exp(torch.clamp(logits[:,-1:], min=-6, max=6))
        logits = logits[:,:-1]*temps
        if get_temps:
            return logits, temps
